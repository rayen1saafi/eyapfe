...
const CreateCoursModal = ({ setopenCourModale, reloadPage, setReloadPage }) => {
  ...
  const uploadCoursImage = async () => {
    try {
      ...
      const response = await axios.post(
        "https://api.cloudinary.com/v1_1/dp1w5pvs0/upload",
        formData
      );
  ...
  const handleSubmit = async () => {
    ...
    if (
      newCours.titre?.length >= 1 &&
      newCours.description?.length >= 1 &&
      newCours.instructor_id
    ) {
      try {
      ...
      <div className="create-cours-modal-container">
        <div className="create-cours-modal-content">
          <div className="c-cours-m-title">
            ...
            <svg
              onClick={() => !setopenCourModale(false)}
          ...
          <div className="cours-modal-button">
            <button
              className="cours-close"
              onClick={() => setopenCourModale(false)}
            >
              Close
            </button>
            <button className="cours-submit" onClick={handleSubmit}>
              Submit
            </button>
          </div>
        </div>
